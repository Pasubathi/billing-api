    ...
    try {
        ...
        async function addExpense() {
            ...
            if (expense_nature == "" || expense_nature == undefined)
                return res.status(200).send({ status:'error', message: "Nature of expense is mandatory" })
            if (expense_amount == "" || expense_amount == undefined)
                return res.status(200).send({ status:'error', message: "Expense Amount is mandatory" })
            if (invoice_date == "" || invoice_date == undefined)
                return res.status(200).send({ status:'error', message: "Invoice Date is mandatory" })
            if (total == "" || total == undefined)
                return res.status(200).send({ status:'error', message: "Total is mandatory" })
