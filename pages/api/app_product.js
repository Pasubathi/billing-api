...
handler.post(async (req, res) => {
 const { user_id, name, category, type, sku, barcode, hsn_code, tax, measure_unit, rate, is_sale, is_purchase, sales_rate, sales_description, purchase_rate, purchase_description, image, description } = req.body;
 if (user_id == "" || user_id == undefined)
    return res.status(200).send({ responseCode: 200,status:"error", message: "User id is mandatory" })
 ...
 if(type && type !='' && type !=undefined)
 {
   ...
   return res.status(200).send({ responseCode: 200,status:"error", message: "Type is mandatory" })
    ...
    await prisma.product__c.create({
        data: data
    });
