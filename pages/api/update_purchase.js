...
export default async function updatePurchase(req, res) {
    await cors(req, res);
    try {
        switch (req.method) {
            case 'POST':
        ...
        async function updatePurchase() {
            const { user_id, purchase_id,  purchase_invoice_no, invoice_date, vendor_gstin, delivery_date, vendor_name, vendor_address, vendor_mob_no, delivery_address, place_of_supply, delivery_to, shipment_preference, tax_type, subtotal, cgst, sgst, shipping_charges, amount_paid, balance_due, total_amount, purchase_note, status, products  } = req.body;
            ...
            try {
                ...
                if (!accountDet) {
                    return res.status(200).send({ status:'error',  message: "Invalid account details" })
                     ...
                     await prisma.purchase__c.update({
                        where:{
