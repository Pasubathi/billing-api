...
export default async function addPrePurchase(req, res) {
    await cors(req, res);
    try {
        switch (req.method) {
            case 'POST':
                return addPrePurchase();
            default:
                return res.status(500).end(`Method ${req.method} Not Allowed`)
        }
        async function addPrePurchase() {
            const { user_id,  purchase_order_no, purchase_date, vendor_gstin, delivery_date, vendor_name, vendor_address, vendor_mob_no, delivery_address, place_of_supply, delivery_to, shipment_preference, tax_type, subtotal, cgst, sgst, shipping_charges, total_amount, purchase_note, status, products  } = req.body;
            ...
            if (purchase_order_no == "" || purchase_order_no == undefined)
                return res.status(200).send({ status:'error', message: "Purchase order no is mandatory" })
            ...
            try {
                ...
                if (!accountDet) {
                    ...
                    const purchase_id = await prisma.pre_purchase__c.create({
                        data:{
                    ...
                    if(Array.isArray(products))
                    {
                        await Promise.all(products.map(async elament =>{
                            await prisma.pre_purchase_order__c.create({
                                data:{
                                    purchase_order_id__c: Number(purchase_id.id),
